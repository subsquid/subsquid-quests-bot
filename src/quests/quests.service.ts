...
export class QuestsService {
  ...
  async claimQuest(questId: number, discordUser: string, discordId: Snowflake): Promise<boolean> {
    return await this.questsRepository.sequelize?.transaction(async (tx) => {
      ...
      if (currentApplicants && currentApplicants.length < questRaw.maxApplicants && !currentApplicants.includes(discordUser)) {
        const applicant = await this.applicantsService.findOrCreateApplicant({ discordHandle: discordUser, discordId: discordId } as Applicant);
